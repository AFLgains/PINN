...
function speed_est(a, config,theta)
    ...
    return MAX_SPEED_INFLUENCE*sigmoid(speed_tf_guess(a)+error)
end
...
function visualise_result_tf_speed(true_speed,config, theta, sess)
    ...
    savefig("results/full_field/speed_tf.png")
end
...
function visualise_result_friction_temp(temp, true_k, config, theta, sess)
    ...
    xlabel("a"); ylabel("K")
    savefig("results/full_field/temp vs k.png")
end
...
function visualise_result_speed_time(time,speed,est_speed_tf)
    ...
    ylim((0,2))
    savefig("results/full_field/time vs speed_tf.png")
end
...
function visualise_result_friction_time(time,true_k,est_k)
    ...
    xlabel("time"); ylabel("k")
    ylim((0,2))
    savefig("results/full_field/time vs k.png")

    figure(figsize=(10,4))
    plot(est_k,true_k,"k.")
    xlabel("est_k"); ylabel("true_k")
    savefig("results/full_field/est_k vs true_k.png")
end
...
function visualise_result_state_time(time,true_h1,true_h2,h1_est,h2_est)
    ...
    savefig("results/full_field/time vs h1.png")

    figure(figsize=(10,4))
    plot(time,true_h2,"k.")
    plot(time,h2_est,"r-")
    xlabel("time"); ylabel("h2")
    savefig("results/full_field/time vs h2.png")
end
