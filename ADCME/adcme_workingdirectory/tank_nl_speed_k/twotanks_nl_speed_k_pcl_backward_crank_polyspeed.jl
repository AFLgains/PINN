...
function create_regression_coeffs(n_coefficients,s)
    nb = length(s)
    regression_matrix = zeros(nb,n_coefficients)
    for  i in 1:n_coefficients
        regression_matrix[:,i] = s.^(i)
    end
...
function speed_est(a, config,theta)
    ...
    regression_matrix = create_regression_coeffs(config,a)
    speed_predictions = regression_matrix*theta
...
function visualise_result_tf_speed(true_speed,config, theta, sess)
    ...
    savefig("results/poly/speed_tf.png")
end
...
function visualise_result_friction_temp(temp, true_k, config, theta, sess)
    ...
    xlabel("a"); ylabel("K")
    savefig("results/poly/temp vs k.png")
end
...
function visualise_result_speed_time(time,speed,est_speed_tf)
    ...
    ylim((0,2))
    savefig("results/poly/time vs speed_tf.png")
end
...
function visualise_result_friction_time(time,true_k,est_k)
    ...
    xlabel("time"); ylabel("k")
    ylim((0,2))
    savefig("results/poly/time vs k.png")

    figure(figsize=(10,4))
    plot(est_k,true_k,"k.")
    xlabel("est_k"); ylabel("true_k")
    savefig("results/poly/est_k vs true_k.png")
end
...
function visualise_result_state_time(time,true_h1,true_h2,h1_est,h2_est)
    ...
    savefig("results/poly/time vs h1.png")

    figure(figsize=(10,4))
    plot(time,true_h2,"k.")
    plot(time,h2_est,"r-")
    savefig("results/poly/time vs h2.png")
end
...
#theta_s = Variable(ones(2))
config_s = 6

speed_coeff_positive = -(Variable([0; 1.0; zeros(config_s-4)]))^2
speed_coeff_unconstrained = Variable([0;1.0])
theta_s = [speed_coeff_unconstrained;speed_coeff_positive]
theta_s = reshape(theta_s,:,1)
