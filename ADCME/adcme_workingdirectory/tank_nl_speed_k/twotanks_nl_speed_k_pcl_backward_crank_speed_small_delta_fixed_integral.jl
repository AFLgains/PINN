...
function visualise_result_tf_speed(true_speed,speed_col,speed_est)
    ...
    savefig("results/small_delta_fi/speed_tf.png")
end
...
function visualise_result_friction_temp(temp, true_k, config, theta, sess)
    ...
    xlabel("a"); ylabel("K")
    savefig("results/small_delta_fi/temp vs k.png")
end
...
function visualise_result_speed_time(time,speed,est_speed_tf)
    ...
    ylim((0,2))
    savefig("results/small_delta_fi/time vs speed_tf.png")
end
...
function visualise_result_friction_time(time,true_k,est_k)
    ...
    xlabel("time"); ylabel("k")
    ylim((0,2))
    savefig("results/small_delta_fi/time vs k.png")

    figure(figsize=(10,4))
    plot(est_k,true_k,"k.")
    xlabel("est_k"); ylabel("true_k")
    savefig("results/small_delta_fi/est_k vs true_k.png")
end
...
function visualise_result_state_time(time,true_h1,true_h2,h1_est,h2_est)
    ...
    savefig("results/small_delta_fi/time vs h1.png")

    figure(figsize=(10,4))
    plot(time,true_h2,"k.")
    plot(time,h2_est,"r-")
    savefig("results/small_delta_fi/time vs h2.png")
end
...
n_col = 101
speed_col = LinRange(0,1.1,n_col)|> collect # Don't go to 1 otherwise matrix becomes less than full rank'
...
#speed_error_rhs = 0.1*(2*sigmoid(Variable(zeros(n_col,1))) -1 )
#total_error_integral = 10*(2*sigmoid(Variable(zeros(1,1))) -1 )
...
println(run(sess,squeeze(speed_tf_error)))
println(run(sess,sum(squeeze(speed_tf_error))))
