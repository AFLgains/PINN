...
function visualise_result_friction_temp(a, true_k, config, theta, sess)
    ...
    xlabel("a"); ylabel("K")
    savefig("results/pcl_fwd/a vs k.png")
end
...
function visualise_result_friction_time(time,true_k,est_k)
    ...
    ylim((0,2))
    savefig("results/pcl_fwd/time vs k.png")

    figure(figsize=(10,4))
    plot(est_k,true_k,"k.")
    xlabel("est_k"); ylabel("true_k")
    savefig("results/pcl_fwd/est_k vs true_k.png")
end
...
function visualise_result_state_time(time,true_h1,true_h2,h1_est,h2_est)
    ...
    savefig("results/pcl_fwd/time vs h1.png")

    figure(figsize=(10,4))
    plot(time,true_h2,"k.")
    plot(time,h2_est,"r-")
    xlabel("time"); ylabel("h2")
    savefig("results/pcl_fwd/time vs h2.png")
end
...
function euler_step(i,h,delt)
    update = h + [(-K_approx[i]*(S_IN[i]*S_IN[i]*(1.0-S_IN[i]*S_IN[i]))*h[1]*h[1] + U_IN[i])*delt; (K_approx[i]*(S_IN[i]*S_IN[i]*(1.0-S_IN[i]*S_IN[i]))*h[1]*h[1] - eta_approx^2 *h[2])*delt]
    return update
