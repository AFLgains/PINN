...
function visualise_result_friction_time(time,true_k,est_k)
    ...
    savefig("results/rm/time vs k.png")

    figure(figsize=(10,4))
    plot(est_k,true_k,"k.")
    xlabel("est_k"); ylabel("true_k")
    savefig("results/rm/est_k vs true_k.png")
end
...
res1,res2 = create_res(h1,h2,K_approx,eta_approx,speed,Uin,dt)
