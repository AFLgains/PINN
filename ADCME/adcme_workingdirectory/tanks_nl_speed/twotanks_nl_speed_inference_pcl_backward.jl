...
function visualise_result_tf_speed(true_speed,config, theta, sess)
    ...
    savefig("results/bck_lin/speed_tf.png")
end
...
function visualise_result_speed_time(time,speed,est_speed_tf)
    ...
    ylim((0,2))
    savefig("results/bck_lin/time vs speed_tf.png")
...
function visualise_result_state_time(time,true_h1,true_h2,h1_est,h2_est)
    ...
    savefig("results/bck_lin/time vs h1.png")

    figure(figsize=(10,4))
    plot(time,true_h2,"k.")
    plot(time,h2_est,"r-")
    xlabel("time"); ylabel("h2")
    savefig("results/bck_lin/time vs h2.png")
end
...
function backward_linearised(i, h_arr)
    ...
    # Forward pass
    Fim1 = [(-K_TRUE[i-1]*speed_tf[i-1]*h[1]*h[1] + U_IN[i-1]); (K_TRUE[i-1]*speed_tf[i-1]*h[1]*h[1] - eta_approx^2 *h[2])]
