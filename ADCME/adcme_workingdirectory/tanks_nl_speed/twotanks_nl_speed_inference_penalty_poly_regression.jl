...
function speed_est(speed,theta)
    output =  theta[2]*speed.^8 .+ theta[3]*speed.^6 .+ theta[4]*speed.^4 .+ theta[5]*speed.^2
    return output
end
...
function visualise_result_tf_speed(true_speed,config, theta, sess)
    ...
    speed_tf = run(sess,speed_est(speed_range,theta))
    plot(speed_range,speed_tf,"r-")
    plot(speed_range,speed_range.^2 .*(1.0.-speed_range.^2),"g--")
    plot(true_speed,true_speed.^2 .*(1.0.-true_speed.^2),"k.")

    xlabel("speed"); ylabel("tf")
    savefig("results/pen_poly/speed_tf.png")
end
...
function visualise_result_speed_time(time,speed,est_speed_tf)
    ...
    ylim((0,2))
    savefig("results/pen_poly/time vs speed_tf.png")
...
function visualise_result_state_time(time,true_h1,true_h2,h1_est,h2_est)
    ...
    savefig("results/pen_poly/time vs h1.png")

    figure(figsize=(10,4))
    plot(time,true_h2,"k.")
    plot(time,h2_est,"r-")
    xlabel("time"); ylabel("h2")
    savefig("results/pen_poly/time vs h2.png")
end
...
theta = Variable(ones(6))
speed_tf = speed_est(speed,theta)
eta_approx = eta_est()
config = 1
